...
class LayoutManager {
    ...
    updateActiveFilters() {
        ...
        Object.entries(this.filters).forEach(([type, values]) => {
            values.forEach(value => {
                ...
                if (type === 'signal') {
                    if (value === '3g') filterName = '3G Network';
                    if (value === '4g') filterName = '4G LTE';
                    if (value === '5g') filterName = '5G Ready';
                    if (value === 'all') filterName = 'Multi-Band';
                }
                
                // Special case for carrier
                if (type === 'carrier') {
                    if (value === 'all') filterName = 'All Carriers';
                }
                
                // Special case for rating
                if (type === 'rating') {
                    if (value === '5') filterName = '5 Stars';
                    if (value === '4') filterName = '4+ Stars';
                    if (value === '3') filterName = '3+ Stars';
                }
    ...
    applySort() {
        const container = document.querySelector('.product-list-view');
    ...
    getRating(productElement) {
        const ratingElement = productElement.querySelector('.product-list-rating span');
        if (ratingElement) {
            return parseFloat(ratingElement.textContent.match(/\d+\.\d+/)[0]);
        }
        return 0;
    }
    ...
    showEmptyState() {
        ...
        if (!document.querySelector('.empty-state')) {
            ...
            emptyState.innerHTML = `
                ...
                <button class="btn-primary px-4 py-2" onclick="layoutManager.clearFilters()">
    ...
    setupQuickView() {
        ...
        document.querySelectorAll('.quickview-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                const item = e.currentTarget.closest('.product-list-item');
                const title = item.querySelector('.product-list-title').textContent;
                ...
                const description = item.querySelector('.product-list-description').textContent;
                const image = item.querySelector('.product-list-image img').src;
                const rating = item.querySelector('.product-list-rating').innerHTML;
    ...
    setupCompare() {
        ...
        document.querySelectorAll('.compare-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                document.querySelector('.product-comparison').scrollIntoView({
                    behavior: 'smooth'
                });
            });
        });
    ...
    setupAddToCart() {
        ...
        document.querySelectorAll('.btn-cart, .btn-add-to-cart, .comparison-table .btn-primary').forEach(btn => {
            btn.addEventListener('click', (e) => {
                e.preventDefault();
                
                // Get product data
                let title;
                if (e.currentTarget.closest('.product-list-item')) {
                    title = e.currentTarget.closest('.product-list-item').querySelector('.product-list-title').textContent;
                } else if (e.currentTarget.closest('.modal-content')) {
                    title = e.currentTarget.closest('.modal-content').querySelector('.product-title').textContent;
                } else if (e.currentTarget.closest('th')) {
                    title = e.currentTarget.closest('th').querySelector('.product-title').textContent;
                }
                
                // Get quantity if in modal
                let quantity = 1;
                const quantityInput = e.currentTarget.closest('.modal-content')?.querySelector('.quantity-selector input');
                if (quantityInput) {
